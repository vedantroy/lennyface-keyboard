...
class FullVersionController: UIViewController
{
	...
	var productsLoaded = 0
	{
		didSet
		{
			...
			if (productsLoaded == 2)
			{
				...
				if (LennyfaceProducts.store.isProductPurchased(fullVersion.productIdentifier))
				{
				...
				} else
				{
					...
					if (IAPHelper.canMakePayments())
					{
						...
						purchaseMessage.text = "Access 50+ faces by upgrading for \(fullVersion.price)."
	...
	override func viewDidLoad()
	{
		...
		if (LennyfaceProducts.store.isProductPurchased(LennyfaceProducts.FullVersion))
		{
		...
		} else
		{
			...
			LennyfaceProducts.store.requestProducts
			{ (success, products) in
				if (success)
				{
	...
	{
		LennyfaceProducts.store.buyProduct(fullVersion)

	}

	func restoreButtonTapped(sender: UIButton!)
	{
		LennyfaceProducts.store.restorePurchases()
	}
...
//		let screenHeight = UIScreen.mainScreen().bounds.height
		...
		purchaseReminder.text = "Thank you for purchasing.\n \n If you have not yet enabled the keyboard or full access, see instructions. \n\n Lennyface keyboard requires full access to verify purchase of the full version. We only have access to Lennyface keyboard's keystrokes and do not record them."
